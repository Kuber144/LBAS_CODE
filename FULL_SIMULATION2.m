% Sensor Network Simulation in MATLAB
% Load Balancing-Aware Clustering Scheme for Heterogeneous Sensor Networks
...
% Plot initial distribution of nodes before any iterations
figure;
scatter(sensorPositions(:, 1), sensorPositions(:, 2), 50, 'filled');
title('Initial Distribution of Nodes');
xlabel('X Position');
ylabel('Y Position');
hold on;
scatter(BS_position(1), BS_position(2), 100, 'r', 'x', 'LineWidth', 2, 'DisplayName', 'Base Station');
legend;
...
function [newClusterHeads, clusterAssignments] = reconfigureClusterHeads(sensorPositions, weights, clusterAssignments, numClusters, numSensors)
            ...
            if any(delta(currentCH, c) > 0) || ...
               ...
               any(weights(moderateOrLowLoadNodes) > weights(currentCH)))
                ...
                if ~isempty(bestCandidate)
                    ...
                    newClusterHeads(c, :) = sensorPositions(currentCH, :); % No change
                end
            else
                newClusterHeads(c, :) = sensorPositions(currentCH, :); % No change
            end
    ...
    % Reassign clusters based on new cluster heads
    for i = 1:numSensors
        if any(~isnan(newClusterHeads(:,1)))
...
for round = 1:rounds
    ...
    % Display information for every 10th iteration
    if mod(round, 10) == 0
        fprintf('Round %d\n', round);
        fprintf('Cluster Heads:\n');
        disp(newClusterHeads);

        % Plot sensor nodes and cluster heads for this iteration
        figure;
        scatter(sensorPositions(:, 1), sensorPositions(:, 2), 50, 'filled');
        hold on;
        scatter(newClusterHeads(:, 1), newClusterHeads(:, 2), 100, 'r', 'filled');
        title(sprintf('Cluster Heads and Assignments after Round %d', round));
        xlabel('X Position');
        ylabel('Y Position');
...
% Final Display of Reconfigured Cluster Heads
fprintf('Final Cluster Heads after %d rounds:\n', rounds);
disp(newClusterHeads);

% Plot sensor nodes and final cluster heads after all rounds
figure;
scatter(sensorPositions(:, 1), sensorPositions(:, 2), 50, 'filled');
hold on;
scatter(newClusterHeads(:, 1), newClusterHeads(:, 2), 100, 'r', 'filled');
title('Final Cluster Heads after All Rounds');
xlabel('X Position');
ylabel('Y Position');
legend('Sensors', 'Cluster Heads');
hold off;
